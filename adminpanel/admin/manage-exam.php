...
<?php 
   $exId = $_GET['id'];
   $selExam = $conn->query("SELECT * FROM exams WHERE id='$exId' ");
   $selExamRow = $selExam->fetch(PDO::FETCH_ASSOC);

   $courseId = $selExamRow['course_id'];
   $selCourse = $conn->query("SELECT name as courseName FROM course WHERE id='$courseId' ")->fetch(PDO::FETCH_ASSOC);
 ?>
...
<div class="app-main__outer">
        <div class="app-main__inner">
                     ...
                     <div class="page-title-heading">
                        <div> MANAGE EXAM
                            <div class="page-title-subheading">
                              Add Question for <?php echo $selExamRow['title']; ?>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                           ...
                           <form method="post" id="updateExamFrm">
                               <div class="form-group">
                                ...
                                <select class="form-control" name="courseId" required="">
                                  <option value="<?php echo $selExamRow['id']; ?>"><?php echo $selCourse['courseName']; ?></option>
                                  <?php 
                                    $selAllCourse = $conn->query("SELECT * FROM course ORDER BY id DESC");
                                    while ($selAllCourseRow = $selAllCourse->fetch(PDO::FETCH_ASSOC)) { ?>
                                      <option value="<?php echo $selAllCourseRow['id']; ?>"><?php echo $selAllCourseRow['name']; ?></option>
                                    <?php }
                                   ?>
                                </select>
                              </div>
                              ...
                              <div class="form-group">
                                ...
                                <input type="" name="examTitle" class="form-control" required="" value="<?php echo $selExamRow['title']; ?>">
                              </div>  

                              <div class="form-group">
                                <label>Exam Description</label>
                                <input type="" name="examDesc" class="form-control" required="" value="<?php echo $selExamRow['description']; ?>">
                              </div>  

                              <div class="form-group">
                                <label>Exam Time limit</label>
                                <select class="form-control" name="examLimit" required="">
                                  <option value="<?php echo $selExamRow['duration']; ?>"><?php echo $selExamRow['duration']; ?> Minutes</option>
                  ...
                  <div class="col-md-6">
                      <?php 
                        $selQuest = $conn->query("SELECT * FROM questions WHERE exam_id='$exId' ORDER BY id desc");                    
                      ?>
                     <div class="main-card mb-3 card">
                                               ...
                                               while ($selQuestionRow = $selQuest->fetch(PDO::FETCH_ASSOC)) { ?>
                                                <tr>
                                                        ...
                                                        <td class="text-center">
                                                         <a rel="facebox" href="facebox_modal/updateQuestion.php?id=<?php echo $selQuestionRow['id']; ?>" class="btn btn-sm btn-primary">Update</a>
                                                         <button type="button" id="deleteQuestion" data-id='<?php echo $selQuestionRow['id']; ?>'  class="btn btn-danger btn-sm">Delete</button>
                                                        </td>
                                                    </tr>
                                               <?php }
                                            }
